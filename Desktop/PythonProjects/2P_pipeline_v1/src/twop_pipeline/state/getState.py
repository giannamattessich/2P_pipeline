...
def get_state_df(facemap_data, camera_times, 
                 treadmill_signal=None, cam_fps=None,
                    filter_treadmill=True, smoothing_kernel=5,
                    movement_percentile=70, min_duration_s=1.0,
                    pupil_data=False, paw_data=False,
                    # what index is motion saved in facemap,
                    use_motsvd = False, motion_indices = {'motion':1},
                    annotate_state=False, annotate_state_with_pupil=False,
                    min_max_norm=True):
    """
    ...
    Args:
        ...
        use_motsvd (bool; default: False): where to extract facemap motion data from keys 
                                            (either from motion key or from a selected SVD)
    Returns:
    ...
    motion_raw = stats.clip_percentiles(despike_cliffs(motion_raw, k=9, z=6.0, only_drops=False))
...
#     # If motion is sampled at the camera frame rate, frame index ≈ round((t - t0) * cam_fps).
    ...
    if treadmill_signal is not None:
        ...
        treadmill_smoothed = stats.clip_percentiles(despike_cliffs(treadmill_smoothed, k=25, z=8.0, only_drops=False))
    ...
    if paw_data:
        ...
        if 'left_paw' in motion_indices.keys():
            ...
            left_paw_raw = stats.clip_percentiles(despike_cliffs(left_paw_raw, k=9, z=6.0, only_drops=False))
            ...
            print('Facemap motion index was not provided for left paw!! Please fix this in "motion_indices" parameter of function call. \
                  The indices should correspond with the index of the SVD index from facemap')
        if 'right_paw' in motion_indices.keys():
            ...
            right_paw_raw = stats.clip_percentiles(despike_cliffs(right_paw_raw, k=9, z=6.0, only_drops=False))
            ...
            print('Facemap motion index was not provided for right paw!! Please fix this in "motion_indices" parameter of function call. \
                  The indices should correspond with the index of the SVD index from facemap')
    ...
    if annotate_state_with_pupil:
        facial_motion_df['state'] = [None] * len(facial_motion_df)
        facial_motion_df = facial_motion_df.apply(lambda x: annotate_state_pupil(x), axis=1) 
